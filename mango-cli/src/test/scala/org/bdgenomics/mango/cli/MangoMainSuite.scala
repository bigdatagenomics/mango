...
import org.bdgenomics.adam.cli.MangoMain.defaultCommandGroups
import org.bdgenomics.mango.cli.MakeGenome
import org.scalatest.FunSuite

class MangoMainSuite extends FunSuite {
  ...
  test("module provides default command groups") {
    val injector = Guice.createInjector(new MangoModule())
  ...
  test("inject default command groups when called via main") {
    ...
    assert(out.contains("MakeGenome"))
  }

  test("command groups is empty when called via apply") {
    val stream = new ByteArrayOutputStream()
    Console.withOut(stream) {
      new MangoMain(List.empty)(Array())
    }
    val out = stream.toString()
    assert(out.contains("Usage"))
    assert(!out.contains("MakeGenome"))
  ...
  test("single command group") {
    val stream = new ByteArrayOutputStream()
    Console.withOut(stream) {
      new MangoMain(List(CommandGroup("SINGLE COMMAND GROUP", List(MakeGenome)))).apply(Array())
    }
    val out = stream.toString()
    assert(out.contains("Usage"))
  ...
  test("add new command group to default command groups") {
    ...
    Console.withOut(stream) {
      ...
      new MangoMain(commandGroups)(Array())
    }
    val out = stream.toString()
    assert(out.contains("Usage"))
  ...
  test("module restores default command groups when called via apply") {
    ...
    Console.withOut(stream) {
      val injector = Guice.createInjector(new MangoModule())
      val commandGroups = injector.instance[List[CommandGroup]]
      new MangoMain(commandGroups).apply(Array())
    }
    val out = stream.toString()
    assert(out.contains("Usage"))
    assert(out.contains("MakeGenome"))
    ...
    Console.withOut(stream) {
      val module = new AbstractModule with ScalaModule {
        override def configure() = {
          bind[List[CommandGroup]].toInstance(List(CommandGroup("SINGLE COMMAND GROUP", List(MakeGenome))))
        }
      }
