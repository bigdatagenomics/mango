...
'../SequenceStore');var _SequenceStore2 = _interopRequireDefault(_SequenceStore);var _RemoteRequest = require(

'../RemoteRequest');var _RemoteRequest2 = _interopRequireDefault(_RemoteRequest);var _utils = require(
...
var createFromReferenceUrl = function createFromReferenceUrl(remoteSource) {
  // Local cache of genomic data.
  var _contigList = remoteSource.getContigList();
  ...
  function normalizeRange(range) {
    if (_contigList.indexOf(range.contig) >= 0) {
      return _q2['default'].Promise.resolve(range);}

    var altContig = _utils2['default'].altContigName(range.contig);
    if (_contigList.indexOf(altContig) >= 0) {
      return _q2['default'].Promise.resolve({ 
        contig: altContig, 
        start: range.start, 
        stop: range.stop });}


    return _q2['default'].Promise.resolve(range); // cast as promise to conform to TwoBitDataSource
  }
  ...
  var o = { 
    ...
    contigList: function contigList() {return _contigList;}, 
...
function create(data) {
  ...
  return createFromReferenceUrl(new _dataSequence2['default'](new _RemoteRequest2['default'](urlPrefix), contigList));}
