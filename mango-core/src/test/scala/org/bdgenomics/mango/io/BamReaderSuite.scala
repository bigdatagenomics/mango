...
class BamReaderSuite extends MangoFunSuite {
  ...
  test("Should load local bam file") {
    ...
    val data = BamReader.load(bamFile, Some(Iterable(region)), true)._2
  ...
  test("Fails on files that are not bam or sam") {
    ...
    val thrown = intercept[Exception] {
      BamReader.load("myfile.vcf", Some(Iterable(region)), true)._2
    }
  ...
  test("Should return no data on empty dictionary") {
    ...
    val data = BamReader.load(invalidSamFile, Some(Iterable(region)), true)._2
    assert(data.size == 0)
  }

  test("Should load local sam file") {
    val region = new ReferenceRegion("1", 26472780L, 26472790L)
    val data = BamReader.load(samFile, Some(Iterable(region)), true)._2
    val x = data.toArray
    assert(x.length == 1)
  }
  ...
  sparkTest("Should load data from HDFS using Spark") {
    ...
    val data = BamReader.loadHDFS(sc, bamFile, Some(Iterable(region)))._2

    val data2 = BamReader.load(bamFile, Some(Iterable(region)), true)._2
  ...
  test("Should load http bam file") {
    ...
    val count = BamReader.load(bamString, Some(Iterable(region)), false)._2.size
