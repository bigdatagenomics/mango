...
import org.bdgenomics.mango.converters.{ AlignmentRecordConverterGA4GH, GA4GHConverter }
...
// metric variables
object AlignmentTimersGA4GH extends Metrics {
 ...
 * @see KTiles
 */
class AlignmentRecordMaterializationGA4GH(@transient sc: SparkContext,
                                          files: List[String],
  ...
  def getCoverage(region: ReferenceRegion): Map[String, Array[PositionCount]] = {
    ...
    AlignmentTimersGA4GH.getCoverageData.time {
...
object AlignmentRecordMaterializationGA4GH extends Logging {
  ...
  def load(sc: SparkContext, fp: String, regions: Option[Iterable[ReferenceRegion]]): AlignmentRecordRDD = {
    if (fp.endsWith(".adam")) loadAdam(sc, fp, regions)
      ...
      try {
        AlignmentRecordMaterializationGA4GH.loadFromBam(sc, fp, regions)
  ...
  def loadFromBam(sc: SparkContext, fp: String, regions: Option[Iterable[ReferenceRegion]]): AlignmentRecordRDD = {
    AlignmentTimersGA4GH.loadBAMData.time {
  ...
  def loadAdam(sc: SparkContext, fp: String, regions: Option[Iterable[ReferenceRegion]]): AlignmentRecordRDD = {
    AlignmentTimersGA4GH.loadADAMData.time {
      val pred =
        if (regions.isDefined) {
