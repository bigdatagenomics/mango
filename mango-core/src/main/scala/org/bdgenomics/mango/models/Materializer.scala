...
case class Materializer(objects: Seq[LazyMaterialization[_, _]]) {
  ...
  /**
   * Access arbitrary structure by name
   *
   * @param name Name to reference Materialization structure by
   * @return Optional LazyMaterialization Structure
   */
  def get(name: String): Option[LazyMaterialization[_, _]] = {
    objects.find(r => r.getName == name)
  }
